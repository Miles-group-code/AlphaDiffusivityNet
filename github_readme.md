...
*   **Mechanism**: Directly optimizes the values of $\log D(x)$ on a discretized grid. It solves the forward problem using a **differentiable FDM solver** (Thomas algorithm) at every training step.
*   **Key Properties**:
...
*   **Mechanism**: Parameterizes both $\log D(x)$ and $u(x)$ as separate neural networks (`LogDNet` and `LocalOperator`). The networks are trained jointly to minimize a composite loss: $\mathcal{L} = \mathcal{L}_{data} + \mathcal{L}_{physics}$.
*   **Key Properties**:
