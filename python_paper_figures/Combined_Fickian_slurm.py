...
def run_single_trial_internal(args, b0=600, method="pinn", device="cpu"):
    ...
    torch.set_num_threads(1)   # one thread per worker; parallelism is across trials
    ...
    problem = Problem.synthetic(
        ...
        d_profile_params=(0.1, 0.05, float(os.environ.get("FREQ", "3.0"))),
        mu=5.0,
        source_location=0.5,
        b_true=b0,
        m_obs=1, 
...
def main():
    # --- CONFIGURATION (sweep params via env; defaults reproduce the original) ---
    n_trials = int(os.environ.get("N_TRIALS", "100"))
    B0   = int(os.environ.get("B0", "250"))        # source strength: 250 or 1000
    FREQ = float(os.environ.get("FREQ", "3.0"))    # D(x) freq: 3 -> sin(6pi x), 4 -> sin(8pi x)
    tag  = f"b{B0}_t{n_trials}_{int(FREQ)}"         # e.g. b250_t100_3
    ...
    dto_worker = partial(run_single_trial_internal, b0=B0, method="dto", device=device)
    pinn_worker = partial(run_single_trial_internal, b0=B0, method="pinn", device=device)
    bilo_worker = partial(run_single_trial_internal, b0=B0, method="bilo", device=device)

    num_processes = int(os.environ.get("SLURM_CPUS_PER_TASK") or os.environ.get("N_WORKERS") or min(mp.cpu_count() - 1, 12)) 
    ...
    # --- save raw results so the figure can be re-plotted/restyled without recomputing ---
    import pickle
    _data_path = f"CCombined_Fickian_{tag}_data.pkl"
    with open(_data_path, "wb") as _f:
        pickle.dump({"dto_results": dto_results,
                     "pinn_results": pinn_results,
                     "bilo_results": bilo_results,
                     "n_trials": n_trials}, _f)
    print(f"[data dump] raw results -> {_data_path}")
    ...
    plt.tight_layout()
    plt.savefig(f"CCombined_Fickian_{tag}.pdf", dpi=300)
    print(f"Results saved to CCombined_Fickian_{tag}.pdf")
    plt.show()
