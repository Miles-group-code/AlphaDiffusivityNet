...
def run_single_trial_internal(args, b0=600, method="pinn", device="cpu"):
    ...
    problem = Problem.synthetic(
        ...
        d_profile_params=(0.1, 0.05, float(os.environ.get("FREQ", "4.0"))),
        mu=5.0,
        source_location=0.5,
        b_true=b0,
        m_obs=1, 
...
def main():
    ...
    B0   = int(os.environ.get("B0", "1000"))       # source strength: 250 or 1000
    FREQ = float(os.environ.get("FREQ", "4.0"))    # D(x) freq: 4 -> sin(8pi x)
    tag  = f"b{B0}_t{n_trials}_{int(FREQ)}"
    ...
    import pickle
    _data_path = f"CCombined_Ito_{tag}_data.pkl"
    ...
    plt.tight_layout()
    plt.savefig(f"CCombined_Ito_{tag}.pdf", dpi=300)
    print(f"Results saved to CCombined_Ito_{tag}.pdf")
    plt.show()
